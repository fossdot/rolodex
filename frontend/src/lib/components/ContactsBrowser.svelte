<script lang="ts">
  ...
  // Pre-fill from ?q= so the "search the Rolodex instead" nudge can carry the
  // query across from My Contacts.
  let search = get(page).url.searchParams.get('q') ?? '';
    ...
    <div class="flex flex-col items-center justify-center py-20 text-center">
      ...
      {#if scope === 'mine' && search.trim()}
        ...
        <a
          ...
          class="btn-primary mt-4 gap-2"
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            <circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/>
          </svg>
          Search the Rolodex for “{search.trim().length > 24 ? search.trim().slice(0, 24) + '…' : search.trim()}”
        </a>
